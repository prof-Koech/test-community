...
Object.defineProperty(t,"__esModule",{value:!0})
t.setupQuillEditor=function(t,n,i,s,u,c,d,h,g,f,T,p,E,m,x,S){const v={"typeahead enter":{key:"enter",handler:()=>!n()}},b={...o.enterKeyBinding,...v},y=new r.default(t.querySelector(`.${e.QUILL_CONTAINER_CLASS}`),{placeholder:i,scrollingContainer:S,formats:["hashtag","mention","guard","bold"],modules:{mention:{actions:{openTypeahead:h,closeTypeahead:g,triggerUnmentionableToast:f,generateNewWorkflowId:T,fireCustomTexteditorTrackingEvent:p,fireControlInteractionEvent:E},trackingData:x},hashtag:{actions:{openTypeahead:h,closeTypeahead:g,generateNewWorkflowId:T,fireCustomTexteditorTrackingEvent:p,fireControlInteractionEvent:E},trackingData:x},keyboard:{bindings:b},history:{delay:50,maxStack:300}}})
delete y.getModule("keyboard").bindings[l.keys.TAB]
y.clipboard.addMatcher("b,strong",a.removeBoldFormatMatcher)
const A=y.container.querySelector(e.QUILL_EDITOR_SELECTOR)
A.setAttribute("aria-placeholder",i)
A.setAttribute("data-placeholder",i)
A.setAttribute("aria-label",s)
A.setAttribute("role","textbox")
A.setAttribute("aria-multiline",!0)
A.setAttribute("aria-describedby",u)
A.setAttribute("data-test-ql-editor-contenteditable",!0)
c&&y.on("text-change",(()=>{c(y.getContents())}))
d&&y.once("text-change",d)
m&&y.focus()
return y}
...
this.quill.setSelection(s+1,e.default.sources.USER)}onTypeaheadSelect(t,e){const n=this.quill.getSelection(!0).index,[i]=this.quill.getLeaf(n),o=i.text.slice(1),a=(0,s.getSearchHeaderForHashtagTrackingEvent)(o)
if(!i)return
...
delete g.text
let E
if("MENTION_DISABLED_ENTITY"===(null==l?void 0:l.renderStyle)){var m,x
E=(new u).retain(d).delete(T).insert(g.originalText)
null===(m=(x=this.actions).triggerUnmentionableToast)||void 0===m||m.call(x,{name:g.originalText})}else E=(new u).retain(d).delete(T).insert(i.default.CONTENT,{guard:!0}).insert(g.originalText,{mention:g}).insert(i.default.CONTENT,{guard:!0})
this.quill.updateContents(E,e.default.sources.USER)
const S=d+g.originalText.length;(0,s.queueSetSelection)(this.quill,S)
const v=(0,o.getSearchHeaderForMentionsTrackingEvent)(f)
this.actions.fireControlInteractionEvent(`${this.trackingData.controlType}_mention_select`)
this.actions.fireCustomTexteditorTrackingEvent(o.EVENTS.MENTION_ACTION,(0,o.getMentionSuggestionActionSelectEventPayload)(v,g,c),a.TYPEAHEAD_TYPES_MAP.MENTION)
